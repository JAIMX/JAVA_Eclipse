...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        ...
        if (this.y == that.y && this.x < that.x) {
            return ((double) (this.y - that.y) / (that.x - this.x));
        }
    ...
    private class Slopecompare implements Comparator<Point> {
        public int compare(Point a, Point b) {
            double test = slopeTo(a) - slopeTo(b);
            if (test < 0)
                return -1;
            else if (test > 0)
                return +1;
    ...
    public static void main(String[] args) {
        ...
        Comparator<Point> test = point[0].slopeOrder();
        // Arrays.sort(point,Point.slopeOrder());
        System.out.println(test.compare(point[0], point[1]));
