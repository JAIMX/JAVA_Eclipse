...
public class Population {   	
    ...
    public void initual(){
    	...
    	int numPart2=30;
    	int LB=chromosomeLength1/chromosomeLength2-5;
    	int UB=chromosomeLength1/chromosomeLength2+6;
    	...
    	for(int i=0;i<numPart2;i++){
        	...
        	for(int j=0;j<chromosomeLength2-1;j++){
        		part2[j]=(int) (Math.min(Math.random()*(UB-LB)+LB, left-(chromosomeLength2-1-j)*LB));
...
//    	System.out.println(population.size());  	
    ...
    public void addPattern(int number){
    	...
    	for(int i=0;i<number;i++){
        	...
        	HashSet<int[]> patternSet2=new HashSet<int[]>();
        	int count=1;
        	for(int[] ele:patternSet){
        		if(count!=1){
        			patternSet2.add(ele);
    ...
    public void select(){
    	...
    	firstFrontIndex=Math.min(newPopulation.size()-1, populationSize);
    	
//		System.out.println("Get there0!!!");
...
//		System.out.println("Get there1!!!");
    	...
    	//Sort by two goals and calculate the distance for every individualS
...
//    	System.out.println("rIndex="+rIndex);
    	...
    	for(int i=0;i<population.size();i++){
    		population.get(i).setPopulationIndex(i);
    	}
