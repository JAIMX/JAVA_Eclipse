...
public class SubtourReversal {
	...
	private static int[] randomIni() {
		...
		while (true) {
			if (set.size() == nbDots)
				break;
			it = temp[((int) (Math.random() * nbDots))];
			if (set.contains(it)) {
				continue;
			} else if (distance[iniRoute[index - 1] - 1][it - 1] >= MAXDISTANCE) {
				count++;
				continue;
			} else {
				iniRoute[index] = it;
			...
			for (int i = 1; i < nbDots - 1; i++) {
				...
				for (int j = 0; i + j + 2 <= nbDots; j++) {
					...
					if (incre - decre < maxDecre) {
                        ...
                        penalty = 1 - min / max;
                        if ( (penalty) + incre - decre < maxDecre ) {
